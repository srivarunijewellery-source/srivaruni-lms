...
export default function LessonPage() {
  ...
  async function handleNext() {
    setSaving(true)
    // Mark complete (stays completed if already was)
    await supabase.from('lms_progress').upsert({
      assignment_id: assignmentId, staff_id: staff.id, content_id: contentId,
      status: 'completed',
    ...
    if (nextItem) {
      ...
      // All lessons done — go back to course (quiz will be unlocked)
  ...
  const body  = content ? (lang === 'te' && content.body_te  ? content.body_te  : content.body_en)  : ''
    ...
    <>
      ...
      <div style={{ position:'sticky', top:0, zIndex:50, background:'var(--plum)', boxShadow:'0 2px 8px rgba(0,0,0,0.3)' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 16px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0 }}>
            ...
            {isDone && <span style={{ fontSize:11, background:'var(--success-bg)', color:'var(--success-text)', borderRadius:20, padding:'2px 8px', fontWeight:700, flexShrink:0 }}>✓ Done</span>}
          </div>
      ...
      <div style={{ display:'flex', minHeight:'calc(100vh - 61px)', paddingBottom:80 }}>
        ...
        <div style={{ flex:1, padding:'24px 20px', overflowY:'auto', minWidth:0 }}>
          ...
          <h1 style={{ fontSize:21, fontWeight:700, color:'var(--charcoal)', marginBottom:18, lineHeight:1.3,
          ...
          {/* Audio */}
          {content?.audio_url && (
            <div style={{ background:'var(--plum)', borderRadius:12, padding:'14px 16px', marginBottom:18 }}>
          ...
          {/* Body */}
          {body && (
            <div style={{ fontSize:15, lineHeight: lang === 'te' ? 2.1 : 1.8, color:'var(--body)', marginBottom:24,
              fontFamily: lang === 'te' ? 'Noto Sans Telugu,sans-serif' : 'DM Sans,sans-serif' }}>
              {body}
            </div>
          )}
          ...
          {/* Last lesson notice */}
          {isLast && (
            <div style={{ background:'var(--info-bg)', border:'1px solid #BFDBFE', borderRadius:10, padding:'12px 16px', marginBottom:16 }}>
              <p style={{ fontSize:14, fontWeight:600, color:'var(--info-text)',
                fontFamily: lang === 'te' ? 'Noto Sans Telugu,sans-serif' : 'DM Sans,sans-serif' }}>
                {lang === 'te' ? '🎯 ఇది చివరి పాఠం! అన్ని పాఠాలు పూర్తయిన తర్వాత మాడ్యూల్ క్విజ్ అన్‌లాక్ అవుతుంది.' : '🎯 This is the last lesson! The module quiz unlocks after you complete all lessons.'}
              </p>
            </div>
          )}
          ...
          <div style={{ display:'flex', gap:10, marginTop:8 }}>
            ...
            <button onClick={handleNext} disabled={saving}
              className="btn-primary" style={{ flex:2, margin:0 }}>
              {saving ? '...' : isLast
                ? (lang === 'te' ? 'పూర్తి చేయండి — క్విజ్‌కి వెళ్ళు ✓' : 'Finish lessons — Take Quiz →')
                : (lang === 'te' ? 'తదుపరి →' : 'Next →')}
            </button>
          </div>
        </div>
      ...
      <style>{`
        ...
        @media (max-width:640px) {
          .lesson-image-panel { width:100% !important; min-width:unset !important; position:relative !important; top:unset !important; height:260px !important; }
        }
      `}</style>
    </>
  )
